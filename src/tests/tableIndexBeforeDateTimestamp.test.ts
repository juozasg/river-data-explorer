...
describe("searches a timestamp table sorted by date", () => {
	...
	it("works with a 1 row table", () => {
		const singleRowTable = aq.table({ date: [new Date(10)], temp: [10] });
		const dateToFind = new Date(10);
	...
	it("works with a 2 row table", () => {
		const twoRowTable = aq.table({
			date: [new Date(10), new Date(20)],
			temp: [10, 20]
		});
		const dateToFind = new Date(11);
